...
router.get('/', async (req, res) => {
    ...
    async function XploaderPair() {
        ...
        try {
            ...
            const myr = await XpbotsPair.sendMessage(XpbotsPair.user.id, { text: "*POPKID BMX SESSION ID BELOW*" });
                        const mega_url = await upload(fs.createReadStream(auth_path + 'creds.json'), `${randomMegaId()}.json`);

                        const string_session = mega_url.replace('https://mega.nz/file/', 'POPKID~');

                        const sid = string_session;

  const dt = await XpbotsPair.sendMessage(XpbotsPair.user.id, { image: { url: "https://files.catbox.moe/jdco7i.jpg" }, caption: `${sid}` }, { quoted: myr });
